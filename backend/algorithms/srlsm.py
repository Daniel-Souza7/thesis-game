...
import time
import math
from backend.algorithms.utils import randomized_neural_networks
...
class SRLSM:
    ...
    def price(self, train_eval_split=2, stock_paths=None):
        """
        ...
        Args:
            ...
            stock_paths: Pre-generated paths (optional)
        ...
        # Store paths for later prediction
    ...
    def predict_exercise_decisions(self, test_paths):
        """
        Predict exercise decisions for test paths using trained model.
        ...
        """
        nb_paths, nb_stocks, nb_dates_plus_one = test_paths.shape
        nb_dates = nb_dates_plus_one - 1
        disc_factor = math.exp(-self.model.rate * self.model.maturity / nb_dates)

        # Initialize with terminal payoff
        values = self.payoff.eval(test_paths)
        exercise_dates = np.full(nb_paths, nb_dates, dtype=int)

        # Forward pass through the trained model
        for date in range(nb_dates - 1, 0, -1):
            # Current immediate exercise value
            path_history = test_paths[:, :, :date + 1]
            immediate_exercise = self.payoff.eval(path_history)

            # Prepare current state
            current_state = test_paths[:, :, date]
            ...
            # Update exercise dates (only if not already exercised)
            mask = exercise_now & (exercise_dates == nb_dates)
            exercise_dates[mask] = date
